...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="bunnys/cicd_guestbook:${strDockerTag}"
    }
    ...
    stages {
        stage('Checkout') {
            agent { label 'agent1' }
            steps {
                git branch: 'master', url:'https://github.com/qweff/guestbook.git'
            }
        }
        stage('Build') {
            agent { label 'agent1' }
        ...
        stage('Docker Image Build') {
            agent { label 'agent2' }
            steps {
                git branch: 'master', url:'https://github.com/bunnys/guestbook.git'
                sh './mvnw clean package'
                script {
    ...
    post { 
        always { 
            emailext (attachLog: true, body: '본문', compressLog: true
                    , recipientProviders: [buildUser()], subject: '제목', to: 'hojunthegrace@gmail.com')
